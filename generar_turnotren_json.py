...
TURNOS = {
    "manana": {"nombre": "MAÑANA", "entrada": "06:15"},
    "tarde": {"nombre": "TARDE", "entrada": "14:15"},
    "noche": {"nombre": "NOCHE", "entrada": "22:15"},
}
...
def seleccionar_tren_para_turno(directos_hoy, directos_manana, entrada_hhmm, ahora):
    """
    Para entrada a turno se busca el tren anterior a la hora oficial de entrada.
    Si esa hora ya pasó hoy, se calcula para mañana.
    """
    h, m = map(int, entrada_hhmm.split(":"))
    objetivo_hoy = datetime.combine(ahora.date(), dtime(h, m), TZ)

    if objetivo_hoy <= ahora:
        objetivo = objetivo_hoy + timedelta(days=1)
    else:
    ...
    elegido = max(candidatos, key=lambda x: x["salida_dt"])
    elegido = dict(elegido)
    elegido["objetivo_turno"] = objetivo.isoformat(timespec="seconds")
    elegido["entrada_turno"] = entrada_hhmm
    return elegido
...
def preparar_ruta(zf, trips_rows, calendar_rows, calendar_dates_rows, alerts, trip_updates, vehicle_positions, clave, nombre, origen_id, destino_id, origen_nombre, destino_nombre, ahora, salida):
    hoy = ahora.date()
    ...
    turnos = {}
    for clave_turno, info in TURNOS.items():
        tren_turno = seleccionar_tren_para_turno(directos_hoy, directos_manana, info["entrada"], ahora)
        if tren_turno:
...
def main():
    ...
    with zipfile.ZipFile(GTFS_ZIP, "r") as zf:
        ...
        rutas = [
            preparar_ruta(
                zf, trips, calendar, calendar_dates, alerts, trip_updates, vehicle_positions,
