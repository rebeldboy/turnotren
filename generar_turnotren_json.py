...
MARGEN_LLEGADA_ENTRADA_MINUTOS = 15
...
def seleccionar_tren_antes_de_hora(directos_hoy, directos_manana, hora_hhmm, ahora):
    """
    Entrada a turno CASA -> TRABAJO.

    Regla de Javier:
    - No basta con salir antes de la entrada.
    - Hay que llegar a Billabona-Zizurkil con margen para ir andando/cambiarse/dar relevo.
    - Se elige el último tren cuya LLEGADA sea como tarde 15 minutos antes de la hora oficial.

    Ejemplo:
    entrada 14:15 -> llegada máxima 14:00.
    """
    ...
    llegada_maxima = entrada - timedelta(minutes=MARGEN_LLEGADA_ENTRADA_MINUTOS)

    candidatos = [t for t in directos_hoy + directos_manana if t["llegada_dt"] <= llegada_maxima]
    ...
    elegido["margen_llegada_minutos"] = MARGEN_LLEGADA_ENTRADA_MINUTOS
    elegido["criterio_turno"] = f"último tren que llega al menos {MARGEN_LLEGADA_ENTRADA_MINUTOS} min antes"
    return elegido
...
def seleccionar_tren_despues_de_hora(directos_hoy, directos_manana, hora_hhmm, ahora):
    ...
    Regla de Javier:
    - MAÑANA: salida aproximada del turno 14:15.
    - TARDE: salida aproximada del turno 22:15.
    - NOCHE: salida aproximada 06:15, normalmente del día siguiente al inicio del turno.
    - Nunca se propone un tren anterior a esa hora.
    - Se acepta cualquier tren real del GTFS que pare en Billabona-Zizurkil y después en Tolosa:
      Cercanías, Regional, Media Distancia o Largo Recorrido si Renfe lo publica en el GTFS.
    """
    h, m = map(int, hora_hhmm.split(":"))
    ...
    candidatos = [t for t in directos_hoy + directos_manana if t["salida_dt"] >= objetivo]
    ...
    elegido["hora_turno"] = hora_hhmm
    elegido["criterio_turno"] = "primer tren posterior a la salida"
    return elegido
    ...
    for clave_turno, info in turnos_cfg.items():
        ...
        if tren_turno:
            ...
            turnos[clave_turno] = {
                ...
                "tren": aplicar_tiempo_real(tren_turno, tu_turno),
            }
        else:
            turnos[clave_turno] = {
                "nombre": info["nombre"],
...
def main():
    ...
    if not descargar_gtfs(salida):
        resultado = {
            ...
            "rutas": [],
            "regla_seguridad": "Si no hay dato real validado, la app debe mostrar SIN DATO y no inventar horarios."
        }
