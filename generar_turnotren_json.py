...
def seleccionar_tren_despues_de_hora(directos_hoy, directos_manana, hora_hhmm, ahora):
    ...
    REGLA INNEGOCIABLE:
    - MAÑANA = salida del trabajo a las 14:15.
    - TARDE  = salida del trabajo a las 22:15.
    - NOCHE  = salida del trabajo a las 06:15.
    - El tren debe SALIR de Billabona-Zizurkil DESPUÉS o justo a esa hora.
    - Nunca se puede devolver 14:10 para un turno de tarde, ni ningún tren anterior.
    - Si la hora de referencia ya pasó, se usa esa misma hora del día siguiente.
    """
    h, m = map(int, hora_hhmm.split(":"))
    ...
    # Doble seguro: si por cualquier motivo el candidato es anterior, se descarta.
...
def main():
    ...
    if not descargar_gtfs(salida):
        resultado = {
            ...
            "rutas": [],
            "regla_seguridad": "CASA_TRABAJO: llegada <= entrada - 15 min. TRABAJO_CASA: salida >= fin de turno. Si no hay dato real válido, mostrar SIN DATO y no inventar horarios."
        }
